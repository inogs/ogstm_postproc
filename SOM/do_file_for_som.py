...
matrix_2D=np.zeros((jpj,jpi),np.int)
...
for avefile in aveLIST[rank::nranks]:    
    ...
    for var in list(NATIVE_VARS):
        ...
        if var == 'MLD':
            VAR        = ncIN.variables[var].data[:,:]      
            all_matrix[month-1,var_c,:]= VAR[mask200_2D]

        elif var == 'NCL':
            VAR        = ncIN.variables['N1p'].data[0,:,:,:]      

            for jj in range(jpj): 
                for ji in range(jpi): 
                    for jk in range(jpk):
                        if VAR[jk,jj,ji] > 0.05:
                            matrix_2D[jj,ji] = jk
                            break
                           
            all_matrix[month-1,var_c,:]= nav_lev[matrix_2D[mask200_2D]]
	else:
...
for np in range(npoints): 
    if region_name(SUB1d[np]) != 'error':
        ...
        F.write("%g\t" %counter); counter = counter + 1
        F.write("%g\t" %Lon1d[np])
        F.write("%g\t" %Lat1d[np])
