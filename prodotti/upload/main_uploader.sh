...
for type in BIOL CARB NUTR PFTC CO2F; do
     my_prex_or_die "./Phase2_DU_uploader_dataset_006_008_monthly.sh -i $PROD_DIR -t $type -y $YEAR -l $logDir"
done
...
for type in BIOL CARB NUTR PFTC CO2F; do
    my_prex_or_die "./Phase2_DU_uploader_dataset_006_008_daily.sh -i $PROD_DIR -t $type -y $YEAR -l $logDir"
done
...
for type in BIOL CARB NUTR PFTC CO2F; do
#    my_prex_or_die "./Phase2_DU_uploader_dataset_006_008_yearly.sh -i $PROD_DIR -t $type -l $logDir"
done


PROD_DIR=/g100_scratch/userexternal/gbolzon0/RA_24/PRODUCTS/CLIM
my_prex_or_die "./Phase2_DU_uploader_dataset_006_008_clim.sh -i $PROD_DIR -l $logDir"
