...
for dataset in $DATASETS ; do  
   ...
   echo n | copernicusmarine get -i $dataset --filter "*-b20250429_fc*" --dry-run > pippo.json; grep s3_url pippo.json  > $filename
   #VERSION=202211
   ...
   #python3 remover_fromtxt.py -i $filename -o $OUTDIR --nomenclature old
